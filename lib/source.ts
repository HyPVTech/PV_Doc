...
async function createPayloadSource(): Promise<
  Source<{
    metaData: MetaData;
    // biome-ignore lint/suspicious/noExplicitAny: Payload CMS returns dynamic data structures
    pageData: any;
  }>
> {
  ...
  for (const category of categories) {
    ...
    for (const doc of categoryDocs) {
      byId.set(String(doc.id), doc as DocRecord);
    }
    ...
    for (const doc of categoryDocs) {
      ...
      files.push({
        ...
        data: {
          ...
          // biome-ignore lint/suspicious/noExplicitAny: VirtualFile requires flexible data type
        } as any,
        type: "page",
      });
    }
