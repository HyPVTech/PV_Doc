...
/**
 * User with role field (regular User from users collection)
 */
...
/**
 * Type guard to check if user has a role property (regular User vs MCP API key)
 * The MCP plugin adds PayloadMcpApiKey to the user union type which doesn't have a role field
 */
export function hasRole(user: unknown): user is UserWithRole {
  return (
    user !== null &&
    typeof user === "object" &&
    "role" in user &&
