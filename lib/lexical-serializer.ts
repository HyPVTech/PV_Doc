...
/**
 * Lexical node structure for content processing
 */
...
// Regex for matching Vimeo video IDs - defined at module level for performance
const VIMEO_ID_REGEX = /\/(?:video\/)?(\d+(?:\/[\w-]+)?)/;
...
function extractVimeoId(url: string): string | null {
  try {
    ...
    const match = urlObj.pathname.match(VIMEO_ID_REGEX);
    return match?.[1] ?? null;
  } catch {
    return null;
  }
...
export function extractTableOfContents(
  ...
  function extractHeadings(node: LexicalNode | null | undefined): void {
    if (!node) {
      return;
    }
 ...
 */
function extractTextFromNode(node: LexicalNode | null | undefined): string {
  if (!node) {
    return "";
  }
